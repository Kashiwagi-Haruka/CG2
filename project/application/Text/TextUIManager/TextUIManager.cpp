...
void TextUIManager::Initialize() {
    ...
    sprite_->SetAnchorPoint({ 0.5f,1.0f });
    sprite_->SetRotation(0.0f);
    sprite_->SetScale({ SCREEN_SIZE::WIDTH,160.0f });
    sprite_->SetPosition({ SCREEN_SIZE::HALF_WIDTH,SCREEN_SIZE::HEIGHT-32.0f });
...
void TextUIManager::Update() {
    ...
    if (gentlemanTalk_.GetIsTyping() && GentlemanTalk::GetIsDraw()) {
    ...
    //全てのトークを得る
    if (GetTalkTyping()&& gentlemanAlpha_>0.0f || coffeeTriviaAlpha_>0.0f|| edamameTriviaAlpha_>0.0f) {
        spriteAlpha_ = 1.0f;
    } else {
        const float fadeStep = deltaTime / kEdamameFadeDuration_;
        spriteAlpha_ = std::max(0.0f, spriteAlpha_ - fadeStep);
    }

    sprite_->SetColor({ 0.0f,0.0f,0.0f,spriteAlpha_ * 0.5f });
    //スプライトを更新
    sprite_->Update();
}
...
void TextUIManager::Draw() {
    ...
    if (!ChairMenu::GetIsShowMenu() && !LockerMenu::GetIsShowMenu()) {

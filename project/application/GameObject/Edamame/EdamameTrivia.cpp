...
bool EdamameTrivia::isCurrentVoiceFinished_ = true;
...
EdamameTrivia::~EdamameTrivia() { Audio::GetInstance()->SoundUnload(&triviaVoice_); }
...
void EdamameTrivia::Update() {
	...
	if (isEnd_) {
		...
		if (Audio::GetInstance()->IsSoundFinished(triviaVoice_) && triviaNum_ == strings_.size() - 1) {
			count_ = 2;
		}
...
void EdamameTrivia::UpdateVoiceFinishedState() { isCurrentVoiceFinished_ = Audio::GetInstance()->IsSoundFinished(triviaVoice_); }
...
void EdamameTrivia::SetVol(float vol) { Audio::GetInstance()->SetSoundVolume(&triviaVoice_, vol); }
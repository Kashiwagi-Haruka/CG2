...
void WarpPos::Update()
{
    Matrix4x4 child = { 0.0f };

    if (playerCamera_) {
        Vector3 rotate = transform_.rotate/* + playerCamera_->GetRotate()*/;
         child = Function::MakeAffineMatrix(transform_.scale, rotate, transform_.translate);
    }
